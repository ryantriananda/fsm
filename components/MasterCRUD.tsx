...
import { Search, Plus, Filter, Download, Edit2, Trash2, X, Save, Loader2, AlertCircle } from 'lucide-react';
...
const MasterCRUD: React.FC<MasterCRUDProps> = ({ title, description, columns, fields, initialData = [], tableName }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Keep initialData as fallback if fetch fails (e.g., table doesn't exist yet)
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        // Create
        // Remove id if it's empty string to let DB generate UUID
